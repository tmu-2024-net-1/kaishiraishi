...
const letters9 = "法律と風習とによって、ある永劫えいごうの社会的処罰が存在し、かくして人為的に地獄を文明のさなかにこしらえ、聖なる運命を世間的因果によって紛糾せしむる間は、すなわち、下層階級による男の失墜、飢餓による女の堕落、暗黒による子供の萎縮いしゅく、それら時代の三つの問題が解決せられない間は、すなわち、ある方面において、社会的窒息が可能である間は、すなわち、言葉を換えて言えば、そしてなおいっそう広い見地よりすれば、地上に無知と悲惨とがある間は、本書のごとき性質の書物も、おそらく無益ではないであろう。　　　　　　　　　　　一八六二年一月一日オートヴィル・ハウスにおいて";
const letters = letters1;

const titles = ["小説の面白さ", "〔雨ニモマケズ〕", "鮑の水貝", "月夜のでんしんばしら", "青森", "桜の樹の下には", "詩人は辛い", "谷崎潤一郎氏", "レ・ミゼラブル 03 序"];
const authors = ["太宰治", "宮沢賢治", "北大路魯山人", "宮沢賢治", "太宰治", "梶井基次郎", "中原中也", "芥川龍之介", "ヴィクトル・ユゴー"];
...
let counter = 0;
let fadingTexts = []; // テキストとその透明度を保持
let titleFadingTexts = []; // タイトル用の文字情報を保持
let authorFadingTexts = []; // 著者名用の文字情報を保持

const siteTitle = "私小説"; // サイト名
let siteTitleX, siteTitleY;
let siteFadingTexts = []; // サイト名の文字情報を保持
const letterSpacing = 48; // 文字間隔を設定
let startTime;
let displayTitle = false; // タイトル表示のフラグ

let endTime = 0; // 終了時間を記録する変数
let displayEndText = false; // 終了テキスト表示フラグ
let initialDisplayDone = false; // 初期表示が完了したかどうかのフラグ
